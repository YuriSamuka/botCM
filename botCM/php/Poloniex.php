...
class Poloniex
{
    ...
    public function get_availableAccountBalances() {
        return $this->query(
            array(
    ...
    public function buy($pair, $rate, $amount, $typeOrder = null) {
		$query = [
			'command' => 'buy',
			'currencyPair' => strtoupper($pair),
			'rate' => $rate,
		...
		switch ($typeOrder) {
			case FILLORKILL_ORDER:
				$query['fillOrKill'] = 1;
				break;
			case IMMEDIATEORCANCEL_ORDER:
				$query['immediateOrCancel'] = 1;
				break;
			case POSTONLY_ORDER:
				$query['postOnly'] = 1;
				break;
    ...
    public function sell($pair, $rate, $amount, $typeOrder = null) {
		$query = [
			'command' => 'sell',
			'currencyPair' => strtoupper($pair),
			'rate' => $rate,
    ...
    public function get_chart($pair, $star, $end, $period) {
        $trades = $this->retrieveJSON($this->public_url.'?command=returnChartData&currencyPair=' . strtoupper($pair) . '&start=' . $star . '&end=' . $end . '&period=' . $period);
        return $trades;
    }
...
//    public function get_availableAccountBalances() {
//        $volume = $this->retrieveJSON($this->public_url.'?command=returnAvailableAccountBalances');
//        return $volume;
//    }
